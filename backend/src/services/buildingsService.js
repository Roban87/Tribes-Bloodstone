...
export const buildingsService = {
  ...
  async updateResourceRate(kingdomId, buildingType, rule) {
    if (buildingType === 'farm' || buildingType === 'mine') {
      const resource = buildingType === 'farm' ? 'food' : 'gold';
      await resourceRepo.updateResourceRate(kingdomId, resource, rule[buildingType].generation);
    }
  },
  ...
  async addBuilding(type, kingdomId) {
    ...
    const addNewBuildingData = await buildingsRepo
      .addNewBuilding(type, kingdomId, buildRules[type].price);
  ...
  async upgradeBuilding(buildingId, kingdomId) {
    ...
    await buildingsRepo.upgradeBuilding(
      building.id, upgradeRules[building.type].hp, upgradeRules[building.type].time,
    );
