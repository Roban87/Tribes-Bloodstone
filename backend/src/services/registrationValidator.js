...
export const registerValidator = {    
    validateUsernameAndPassword(username, password) {
        ...
        }else if(username !== undefined && password !== undefined && password.length < 8){
            return {
    ...
    async userNameTaken(username){
        let userSelected = await registrationRepo.getUserByUsername(username);
    ...
    async registUser(username, password, kingdomname){
        ...
        if(kingdomname !== undefined){
            ...
            const insertNewUser = await registrationRepo.insertNewUserWithKingdom(username, hashedPassword, kingdomname);        
            return {
            ...
            let hashedPassword =  await bcrypt.hash(password, saltRounds);
            const insertNewUser = await registrationRepo.insertNewUserWithKingdom(username, hashedPassword, username);                
            return {
                userId: insertNewUser.results[0].id,
                username: insertNewUser.results[0].username,
                kingdomId: insertNewUser.results[0].kingdom_id
